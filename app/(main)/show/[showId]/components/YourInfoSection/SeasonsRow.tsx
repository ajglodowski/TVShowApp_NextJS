...
export default function SeasonsRow({ userId, currentSeason, totalSeasons, showId, updateFunction }:
  ...
  const notCurrentSeasonStyle = `py-1 px-2 m-1 rounded-md text-sm outline outline-white hover:bg-white hover:text-black`;
  const currentSeasonStyle = `py-1 px-2 m-1 rounded-md text-sm bg-white text-black`;
  ...
  return (
    <div>
      ...
      <div className="">{renderSeasonButtons()}</div>
    </div>
  );
};
